        ...
        <div className="flex border-b border-gray-100 bg-white">
          {Array.from({ length: columns }).map((_, i) => (
            <div key={i} className="flex-1 px-3 py-2">
              <div className="animate-pulse rounded bg-gray-200 h-3 w-16 sm:w-20 transition-opacity duration-200" />
            </div>
          ))}
        </div>
        {/* rows */}
          ...
          >
            {Array.from({ length: columns }).map((_, ci) => (
              <div key={ci} className="flex-1 px-3 py-2">
                <div
                  className={`animate-pulse rounded bg-gray-200 h-3 transition-opacity duration-200 ${
